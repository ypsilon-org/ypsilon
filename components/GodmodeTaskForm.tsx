...
export default function GodmodeTaskForm({
  ...
  const selectedColor = unitId
    ? (UNIT_COLORS[units.find((u) => u.id === unitId)?.name ?? ""] ?? "#C8A84B")
    : "#C8A84B";
    ...
    >
      <style>{`
        ...
        .gf-btn {
          ...
          background: #C8A84B; border: 1px solid #C8A84B;
          padding: .85rem 2rem; cursor: pointer;
          transition: box-shadow .3s ease, opacity .2s; width: 100%;
        }
